...
SELECT V2.VIDEO_TITLE, MAX(U1.AGE), MIN(U1.AGE) 
FROM `youtube`.`User_Video_Relation` V1, `youtube`.`User` U1, `youtube`.`Video` V2,
(SELECT VIDEO_ID, COUNT(VIDEO_ID) AS MOST_COMMENTS 
FROM `youtube`.`Video_Comment`
WHERE VIDEO_ID IN (SELECT VIDEO_ID FROM `youtube`.`Video` WHERE USER_ID = (
	SELECT USER_ID FROM `youtube`.`User`
	WHERE USER_NAME = 'taylorswift'
))
GROUP BY VIDEO_ID
HAVING MOST_COMMENTS >= (
	SELECT COUNT(VIDEO_ID) AS MOST_COMMENTS FROM `youtube`.`Video_Comment`
	WHERE VIDEO_ID IN (SELECT VIDEO_ID FROM `youtube`.`Video` WHERE USER_ID = (
		SELECT USER_ID FROM `youtube`.`User`
		WHERE USER_NAME = 'taylorswift'
	))
	GROUP BY VIDEO_ID
    ORDER BY MOST_COMMENTS DESC LIMIT 1
)
ORDER BY MOST_COMMENTS DESC) AS T1
WHERE V1.VIDEO_ID = T1.VIDEO_ID
AND V1.User_ID = U1.User_ID
AND V1.VIDEO_ID = V2.VIDEO_ID
GROUP BY V2.VIDEO_TITLE;
...
-- EXPLANATION
-- The User_Video_Relation table is a bridge between user and video where interaction of user with a video is defined
-- in terms of like, view and interest.

-- Here we have joined 4 tables. One of them is a subquery in a form clause acting as a table. This subquery returns
-- a table that contains videos with most comments produced by user 'taylorswift'. Within the subquery we have a having clause
-- which contains another subquery that is used to ensure that if there is tie between two videos for most commented
-- than it will return both of them. 

-- The other 3 tables joined are User, User_Video_Relation and Video. THe join is required as we need to find users who 
-- watched the most commented videos and we have to get the video title and max and min age of the users.
