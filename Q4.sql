...
SELECT 
    K1.Keyword_Name,
    AVG(V2.Comment_Sentiment) AS Avg_Sentiment
FROM
    `youtube`.`Keywords` K1,
    `youtube`.`Video_Keyword_Relation` V1,
    `youtube`.`Video_Comment` V2
WHERE
    K1.Keyword_ID = V1.Keyword_ID
...
-- EXPLANATION
-- Keywords table contains keyword_id and keywords. The Video_Keyword_Relation table is bridge table between Video
-- and Keywords

-- Here we are joining three tables Keywords, Video_Keyword_Relation and Video Comment. We are getting all the keywords
-- and associated videos to it and all the Video Comments associated to the Video.alter

-- Finally we group the joined table using the Keyword_Name and calculating the average sentiment for that keyword name
-- We also perform the order by avg_sentiment as per the requirement in the question.
