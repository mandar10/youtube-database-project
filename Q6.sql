...
SELECT U1.USER_NAME, T1.Channel_Name, COUNT(UC1.Channel_ID) AS SUBSCRIBER 
FROM `youtube`.`User_Channel_Subscription` UC1 RIGHT JOIN
(SELECT C1.Channel_ID, C1.Channel_Name, COUNT(DISTINCT(WEEK(VIDEO_UPLOAD_DATE))) AS ROW_WEEK
FROM `youtube`.`Video` V1, `youtube`.`Channel` C1
WHERE V1.USER_ID IN(
	SELECT USER_ID FROM `youtube`.`User` 
	WHERE IS_CREATOR = 1 AND Address LIKE '%USA%'
)
AND C1.CHANNEL_ID = V1.CHANNEL_ID
AND V1.VIDEO_UPLOAD_DATE BETWEEN DATE_FORMAT(NOW() - INTERVAL 1 MONTH, '%Y-%m-01') AND DATE_FORMAT(NOW() ,'%Y-%m-01')
GROUP BY C1.Channel_Name
HAVING ROW_WEEK = WEEK(LAST_DAY(now() - INTERVAL 1 MONTH)) - WEEK(DATE_FORMAT(CURDATE() - INTERVAL 1 MONTH,'%Y-%m-01')) + 1) AS T1
ON T1.Channel_ID = UC1.Channel_ID,
`youtube`.`User` U1, `youtube`.`Channel` C1
WHERE T1.CHANNEL_ID = C1.CHANNEL_ID
AND U1.USER_ID = C1.USER_ID
GROUP BY U1.USER_ID;
...
-- EXPLANATION
-- We have first performed right join on 2 tables. First one is the User_Channel_Subscription which is required as we need the 
-- user subscriber count. THe second table is a subquery in the form clause which is used to get user living in the US
-- and checking id they have posted at least every week of last month. The reason for right is if there are 0 subscribers
-- to the channel it would still be in the result.

-- Then we have performed join again on the above combined table with other tables like User & Channel as we have to 
-- print the user name and channel name in the final result

-- To check if they have posted at least every week of last month we have used COUNT DISTINCT function using GROUP BY.
-- We convert the video_upload_date to week uploaded and check if for a user are there distinct specific number of entries.
-- For e.g. in January 2023 we have 5 weeeks. So we check if there are 5 distinct week uploaded entries for any user.

-- We also keep a check if the video posted was from last month only as a safe check.
