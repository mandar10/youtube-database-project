-- QUERY
SELECT s1.Sponsor_Name, s1.Sponsor_Phone , SUM(s2.Sponsorship_Amount) as Total_Sponsorship
FROM `youtube`.`Sponsor` s1, `youtube`.`Video_Sponsorship` s2
WHERE s1.Sponsor_ID=s2.Sponsor_ID
GROUP BY s2.Sponsor_ID 
HAVING Total_Sponsorship>=(
	SELECT SUM(s2.Sponsorship_Amount) as Total_Sponsorship
	FROM `youtube`.`Sponsor` s1, `youtube`.`Video_Sponsorship` s2
	WHERE s1.Sponsor_ID=s2.Sponsor_ID
	GROUP BY s2.Sponsor_ID
    ORDER BY SUM(Sponsorship_Amount) DESC LIMIT 1
);
...
-- EXPLANATION
-- Video Sponsorship table is the bridge table between Video & Sponsor with additional column of Sponsorship which
-- corresponds to the sponsorship the sponsor has given to a particular video

-- We have used a nested subquery in the condition of having clause which ensures that any particular 
-- sponsor having total sponsorship to youtube videos greater than or equal to the highest total sponsorship
-- will be displayed in the result of the query. This subquery handles the corner case where there might be two sponsors
-- with maximum total sponsorship. In this case, there will be two sponsors listed in the result.alter

-- Additionally, we join two tables Sponsor & Video_Sponsorship as we need to display total sponsorship amount
-- along with sponsor's name and phone number.

-- Finally, we need to add GROUP BY on Sponsor_ID as we have to group sponsor by their total sponsorship amount
-- and we have also added SUM function to aggregate the sponsorship amount for grouping.
