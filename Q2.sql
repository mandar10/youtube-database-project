...
(SELECT VIDEO_ID, (SUM(IS_LIKED)/SUM(IS_VIEWED)) AS VIEW_RATIO_LIKE FROM `youtube`.`User_Video_Relation` WHERE VIDEO_ID IN (
	SELECT VIDEO_ID FROM `youtube`.`Video` WHERE Channel_ID IN( 
		SELECT Channel_ID FROM `youtube`.`Channel` 
		WHERE USER_ID IN (
			SELECT USER_ID FROM `youtube`.`User` where User_Name LIKE 'Marvel Entertainment'
		)
	)
...
-- EXPLANATION
-- The User_Video_Relation has a video and user relation with respect to whether the user liked/viewed/interested 
-- in the video

-- Here we have created a subquery in the form clause which acts a table. The subquery contains further nested subqueries
-- This subquery is use to get Video_ID with corresponding Likes to Views Ratio by getting video ids associated
-- to the channels owned by the users having 'Marvel Entertainment' in their user name. Also gourping is performed
-- by Video_ID as we want to have like-view ratio of videos.

-- This table created dynamic is joined with other tables like Video & Channel as we have to display the Channel Name
-- Video Title & View Like Ratio as all these are part of 3 different tables each.
