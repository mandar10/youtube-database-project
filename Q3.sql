-- QUERY
SELECT C1.Channel_Name, C3.User_Name, C3.Email, C2. PAID_USER, C4.TOTAL_USER
FROM `youtube`.`Channel` C1, `youtube`.`User` C3,
(SELECT Channel_ID, COUNT(Channel_ID) AS PAID_USER 
FROM `youtube`.`User_Channel_subscription`
WHERE Subscription_Type = 1
GROUP BY Channel_ID
ORDER BY Channel_ID) AS C2,
(SELECT Channel_ID, COUNT(Channel_ID) AS TOTAL_USER 
FROM `youtube`.`User_Channel_subscription`
GROUP BY Channel_ID
ORDER BY Channel_ID) AS C4
WHERE C1.Channel_Created_Date = '2023-01-01'
AND C2.PAID_USER >=100
AND C1.Channel_ID = C2.Channel_ID
AND C1.User_ID = C3.User_ID
AND C2.Channel_ID = C4.Channel_ID;
...
-- EXPLANATION
-- THe User_Channel_Subscirption table is a bridge table between User and Channel which contains a additional column 
-- of Subscription_Type. Subscription_Type as 1 defines that the subscription is paid and 0 defines that the
-- Subscription is unpaid.

-- Here we have 2 subqueries in form clause which acts as a table. First one is used to group all channels 
-- by count of paid subscriptions. Second one is used to group all channels by count of total subscription.

-- Both the above tables are joined along with Channel and User table as we need channel and user details along with
-- the total and paid users.alter

-- Additionally, we have a condition on number of paid user should be greater than 100. We also have a condition where
-- we check whether Channel_Created_Date is '2023-01-01'
